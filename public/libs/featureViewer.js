//鼠标高亮
var featureViewer = {
    showInfo: true, //是否开启单击显示属性
    colorHighlight: new Cesium.Color.fromCssColorString("#95e40c"),
    colorSelected: new Cesium.Color.fromCssColorString("#95e40c"),
    ...
    install: function (viewer) {
        ...
        this.onLeftClick = function (event) {
            ...
            // 拾取feature
            // var pickedFeature = self.pickUpFeature(selected, highlighted, event.position);
            ...
            if (viewer.infoBox) {
                ...
                //popup方式显示  
                var cartesian = hchx3d.point.getCurrentMousePosition(viewer.scene, event.position);
                setTimeout(function () {
                    viewer.hchx.popup.show({
                        id: "bim",
                        popup: {
                            html: html,
                            anchor: [0, -15]
    ...
    //拾取Feature
    // lastSelectTileset: null,
    // pickUpFeature: function (selected, highlighted, position) {
    ...
    //     if (!Cesium.defined(pickedFeature.getProperty)) {
    //         if (setOut) {//已移出则归位
    //             var lng = Cesium.Math.toDegrees(this.lastSelectTilesetDefaultPositon.longitude);
    //             var lat = Cesium.Math.toDegrees(this.lastSelectTilesetDefaultPositon.latitude);
    //             var height = this.lastSelectTilesetDefaultPositon.height - 135;
    //             var position = Cesium.Cartesian3.fromDegrees(lng, lat, height);
    //             var m = Cesium.Transforms.eastNorthUpToFixedFrame(position);
    //             this.lastSelectTileset._root.transform = m;
    //             setOut = false;
    //         }
    //         return undefined;
    //     }
    //     selected.feature = pickedFeature;
    //     gloabSelected = selected;
    ...
    //     }
    //     // Highlight newly selected feature
    //     if (pickedFeature.tileset.basePath.indexOf("handlerModel") == -1) {
    ...
    //     var curSelectTileset = pickedFeature.tileset;

    //     if (curSelectTileset == this.lastSelectTileset) {//本次点击与上次点击同一个
    //         if (setOut) {//已移出则归位
    //             var lng = Cesium.Math.toDegrees(this.lastSelectTilesetDefaultPositon.longitude);
    //             var lat = Cesium.Math.toDegrees(this.lastSelectTilesetDefaultPositon.latitude);
    //             var height = this.lastSelectTilesetDefaultPositon.height - 135;
    ...
    //             setOut = false;
    //         } else {//未移出则移出
    //             var c = pickedFeature.tileset.boundingSphere.center;
    //             var cartographic = Cesium.Cartographic.fromCartesian(c);
    //             var ellipsoid = viewer.scene.globe.ellipsoid;
    //             var xyz = new Cesium.Cartesian3(c.x, c.y, c.z);
    ...
    //             var height = cartographic.height - 135;
    //             this.lastSelectTileset = curSelectTileset;
    //             this.lastSelectTilesetDefaultPositon = wgs84;
    ...
    //                     steplng = steplng + 0.00007;
    //                     var position = Cesium.Cartesian3.fromDegrees(steplng, lat, height);
    //                     var m = Cesium.Transforms.eastNorthUpToFixedFrame(position);
    //                     pickedFeature.tileset._root.transform = m;

    //                     if (steplng >= lng + 0.00029) {
    ...
    showTilesParts: function (pickedObject) {
        ...
        if (pickedObject && Cesium.defined(pickedObject.primitive)
            ...
            if (hchx3d.widget.isActivate(tilesParts)) {
                var parts = hchx3d.widget.getClass(tilesParts);
                if (parts.config.layerWork == pickedObject.primitive._config._layer)
                    return;//当前已激活,并且单击了相同模型时跳出
            }

            hchx3d.widget.activate({
