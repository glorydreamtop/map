		...
		<div>
			<el-dialog :title="dialogTitle" :append-to-body="true" @close='closeDialog' :visible.sync="showFlag" v-model="showFlag"
			 class="newStyleDialog " custom-class="jbqk_add_table1_dialog">
				<programHeaderAdd :dialog-type="dialogType" v-on:showStudes="showStudescode" :dialog-form="dialogForm" v-if="showFlag"></programHeaderAdd>
			</el-dialog>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			lookClick(row) { //查看专业项目基本情况调查表
				this.dialogForm = row;
				this.dialogTitle = '查看专业项目基本情况调查表';
			...
			addClick() { //添加
				this.dialogForm = '';
				this.dialogTitle = '添加专业项目基本情况调查表';
				this.dialogType = 'add';
				this.showFlag = true;
			...
			editClick(row) { //修改
				this.dialogForm = row;
				this.dialogTitle = '编辑专业项目基本情况调查表';
