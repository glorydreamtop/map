<template>
	<div>
		<el-dialog :visible.sync="visible" :close-on-press-escape="false" width="70vw" v-loading="loading2"
		 element-loading-text="文件转码中..." title="资料管理" @close="$emit('update:showDialog', false)">
			<div class="box">
				<el-tree v-loading="loading1" element-loading-text="加载中..." class="tree" :data="folders" :props="defaultProps"
				 @node-click="handleNodeClick"></el-tree>
				<div class="datalist">
					<el-button title="上传" size="small" class="al-icon-tianjia margin-bottom-m" @click="addFile" />
					...
					<el-table :data="tableData" v-loading="loading3">
						...
						<el-table-column fixed="right" label="操作" width="230" class="justify-between">
							<template slot-scope="scope">
								...
								<el-button title="更新" icon="al-icon-xiugai" type="warning" plain @click="editClick(scope.row)"></el-button>
								<el-button title="删除" icon="al-icon-shanchu" type="danger" plain @click="delClick(scope.row)"></el-button>
							</template>
						</el-table-column>
					</el-table>
					<el-pagination class="margin-top-l" :total="currentTotal" small :page-size="5" @current-change="pageChange"></el-pagination>
				</div>
			</div>
		</el-dialog>
	</div>
